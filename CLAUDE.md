...
## House rules (from the user's global CLAUDE.md)
...
src/
  main/           # Electron main process (Node, ESM)
    ...
    appLifecycle.js
    trayIcon.js   # green "S" PNG embedded as base64 — placeholder
    usageController.js   # orchestrates refresh + auth state
    localDataService.js  # reads ~/.claude/readout-*.json
...
assets/
  tray.png        # asset (the live tray icon comes from trayIcon.js)
```
...
- **Source data** (read-only, written by Claude Code itself):
  `~/.claude/readout-cost-cache.json` and `~/.claude/readout-pricing.json`.

## Refresh cadence

- Local cost cache → every **30 seconds** (`localTimer`).
- OAuth quota → every **120 seconds** (`quotaTimer`), only when signed in.
- Rate-limit (`429`) → respects `Retry-After`, pauses quota refresh until
  `rateLimitedUntil`.

## Tray + window behavior (already implemented)

- Tray icon: 16×16 green "S" generated from a base64 PNG in `trayIcon.js`.
  Replace this when a final icon is provided.
- **Double-click** the tray icon → main window shows.
- **Right-click** → context menu: *Mostrar aplicativo* / *Configurações* /
  *Sair*. Built in `appLifecycle.buildTrayMenuTemplate`.
...
The user runs these — Claude should not run them without asking.
...
See `ROADMAP.md`. Highlights: real tray icon, electron-builder packaging
(`.exe` / `.msi`), code signing, Windows autostart on login, optional
in-app handling of the OAuth redirect (today it's copy-paste).
