...
When a project change makes this document or `AGENTS.md` inaccurate, edit both
...
src/
  ...
  renderer/
    ...
    viewState.js
  shared/
    diagnostics.js # redacts tokens/OAuth values from logs
...
- Rate-limit (`429`) → respects `Retry-After`, pauses quota refresh until
  `rateLimitedUntil`.
- Scope error (`403`) → sets `state.needsReauth = true` and `quotaError =
  'error.scope_insufficient'`; token is **not** cleared. Renderer shows a
  re-auth button that re-starts the PKCE flow. Cleared on next successful
  quota fetch or when sign-in completes.
...
- When touching behavior reflected in `CLAUDE.md` or `AGENTS.md`, update both
  files together so future agents get accurate project context.

## Things the user has flagged as "later"

See `ROADMAP.md`. Highlights: optional in-app handling of the
